...
# - By default all regions are and products are selected
...
# dash app instantiation is a simpler line than Flask app:
# app = Flask(__name__)

# 2. Load the toy-sales.csv dataset
toys_df = pd.read_csv("./csv/toy-sales.csv", parse_dates=["date"])
...
units_by_prod_df = toys_df.groupby("product", as_index=False)["units"].mean().sort_values(by="units", ascending=False)
...
bar_chart = px.bar(
    ...
    x="product",
    y="units",
    title="Avg Unit Sales by Product"
)
...
heatmap_pivoted_df = heatmap_df.pivot(index="region", columns="product", values="revenue")

# 9. Plot the heatmap from the heatmap_pivoted_df
heatmap_fig = px.imshow(
    heatmap_pivoted_df,
    labels=dict(x="Product", y="Region", color="Revenue"),
    title="Revenue: Product by Region",
    aspect="auto",
)
...
unique_regions = sorted(list(toys_df["region"].unique()))
...
#     id, options, value, multi, clearable and placeholder props
        dcc.Dropdown(
            ...
            options=region_menu_options, # the dropdown menu choices
            value=unique_regions,
            multi=True, # allows user to select more than one region
            clearable=False, # must have at least ONE region selected
            placeholder="Select one or more regions" # prompts the user
        ),
...
#     id, options, value, multi, clearable and placeholder props
        dcc.Dropdown(
            ...
            options=product_menu_options, # the dropdown menu choices
            value=unique_products,
            multi=True, # allows user to select more than one region
            clearable=False, # must have at least ONE region selected
            placeholder="Select one or more products" # prompts the user
        ),

# 21. Assign label for the Line Chart (called Line Mode as it is interactive)
        boot.Label("Line Mode", className="mt-3"),
...
# min_date_allowed, max_date_allowed, start_date, end_date, display_format
    ...
    body=True,
    className="shadow-sm rounded-3" # add drop shadow w rounded corners
)

# Layout App Dashboard  with its 3 Charts

# 25. Define a layout for the app, wrapping all in bootstrap container
...
# 26. Define a Col
                boot.Col(
                    [
                        html.H2("Acme Toys Sales Dashboard (Pt. 4 - Full UI)"),
                        html.P("2 dropdowns; 1 radio set; 1 date picker; 3 responsive charts (Line, Bar, Heatmap)"),
                    ], 
...
# 30. make a Col to hold the graphs
                boot.Col(
                    [
                        boot.Card(boot.CardBody([ dcc.Graph(figure=line_chart,
                            id="revenue-line-chart") ] ), 
                            className="shadow-sm rounded-3 mb-3"),
                        boot.Card(boot.CardBody([ dcc.Graph(figure=bar_chart,
                            id="product-bar-chart") ] ), 
                            className="shadow-sm rounded-3 mb-3"),
                        boot.Card(boot.CardBody([ dcc.Graph(figure=heatmap_fig,
                            id="heatmap-chart") ] ), 
                            className="shadow-sm rounded-3 mb-3"),
                    ],
                    md=8,
...
def update_line_chart(selected_regions, selected_products, radio_btn_mode, start_date, end_date):
    ...
    mask = toys_df["region"].isin(selected_regions) & toys_df["product"].isin(selected_products) & (toys_df["date"] >= pd.to_datetime(start_date)) & (toys_df["date"] <= pd.to_datetime(end_date))
     
    # 36. make a copy of the df for use in rebuilding line chart
    # df.loc[mask] select only those rows where the region is selected
    selected_df = toys_df.loc[mask] # filter out unselected regions
    
    # 37. update / redeclare the rev_by_date_df using only selected regions
    rev_by_date_df = selected_df.groupby("date", as_index=False)["revenue"].sum()

    # 38. check which radio btn is selected and use that to make the line chart
    if radio_btn_mode == "agg": # if Aggregate btn is selected, make unified line
        rev_by_date_df = ( selected_df.groupby("date", as_index=False)["revenue"].sum() )
        
        # 39. update / redeclare the line chart using the updated rev_by_date_df df
        line_chart = px.line(rev_by_date_df, x="date", y="revenue",
            title="Toy Sales Revenue, Selected Regions 2023",
            markers=True # puts dots on the points connected by the line      
        )
        
    # 40. else check if the radio button choice is region
    elif radio_btn_mode == "region":
        rev_by_date_df = ( selected_df.groupby(["date","region"], as_index=False)["revenue"].sum().sort_values(["date","region"]) )
        # 41. update / redeclare the line chart using the updated rev_by_date_df df
        line_chart = px.line(rev_by_date_df, x="date", y="revenue",
            title="Toy Sales Revenue, Selected Regions 2023", color="region",
            markers=True # puts dots on the points connected by the line      
        )
        
    # 42. else the radio button choice must be product
    else: # mode is "product"
        rev_by_date_df = ( selected_df.groupby(["date","product"], as_index=False)["revenue"].sum().sort_values(["date","product"]) )
        # 43. update the line chart
        line_chart = px.line(rev_by_date_df, x="date", y="revenue",
            title="Toy Sales Revenue, Selected Regions 2023", color="product",
            markers=True # puts dots on the points connected by the line      
        )
    ...
    units_by_prod_df = selected_df.groupby("product", as_index=False)["units"].mean().sort_values(by="units", ascending=False)

    # 46. update / redeclare the bar chart using the updated rev_by_date_df df
    bar_chart = px.bar(units_by_prod_df, x="product", y="units",
        title="Avg Unit Sales by Product",
    )
    bar_chart.update_layout(margin=dict(l=20,r=20,t=60,b=30))
